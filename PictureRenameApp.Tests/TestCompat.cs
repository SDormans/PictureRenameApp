...
namespace PictureRenameApp.Tests
{
    ...
    public static class Assert
    {
        ...
        {
            if (obj is not null) throw new XunitException("Assert.Null() Failure");
        }
        ...
        {
            if (collection.Any()) throw new XunitException("Assert.Empty() Failure: collection is not empty");
        }
        ...
        public static void Equal<T>(T expected, T actual)
        {
            if (!EqualityComparer<T>.Default.Equals(expected, actual))
                throw new XunitException($"Assert.Equal() Failure: expected {expected}, got {actual}");
        }
    }
